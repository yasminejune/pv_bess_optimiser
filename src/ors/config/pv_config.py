...
class PVSiteConfig:
    ...
    def __post_init__(self):
        ...
        if not (0 <= self.module_efficiency <= 1):
            raise ValueError(
                f"module_efficiency must be in [0, 1]: {self.module_efficiency}"
            )
        if not (0 <= self.inverter_efficiency <= 1):
            raise ValueError(
                f"inverter_efficiency must be in [0, 1]: {self.inverter_efficiency}"
            )
        if not (0 <= self.performance_ratio <= 1):
            raise ValueError(
                f"performance_ratio must be in [0, 1]: {self.performance_ratio}"
            )
        ...
        if not (0 <= self.clipping_loss_factor <= 1):
            raise ValueError(
                f"clipping_loss_factor must be in [0, 1]: {self.clipping_loss_factor}"
            )
...
# Based on project specifications for BURST deployment scenarios
PV_SITE_CONFIGS = {
    ...
    if isinstance(site_type, str):
        try:
            ...
            raise ValueError(
                ...
                f"Valid options: {[t.value for t in SiteType]}"
            ) from e
